...
const TicketsContainer = ({ handleUsingFilters, isUsingFilters, setIsFilterClear, isFilterClear }) => {
    ...
    const [menuLocation, setMenuLocation] = useState({ currentTitle: '', data: [] })
    const [menuStatus, setMenuStatus] = useState({ currentTitle: '', data: [] })
    const [menuCustomer, setMenuCustomer] = useState({ currentTitle: '', data: [] })

    const mappedMenuStatus = mapTicketsDataAndChangeState(menuStatus.data)

    const handleClickMenuDate = (e) => setMenuDate(prev => ({ ...prev, currentTitle: e.target.textContent }))
    const handleClickMenuLocation = (e) => setMenuLocation(prev => ({ ...prev, currentTitle: e.target.textContent }))
            ...
            try {
                ...
                .then(res => {
                    if (res.data.tickets?.length){
                        ...
                        setMenuLocation(prev => ({ ...prev, data: getMenuItemsByValue(res.data.tickets, 'room_number') }));
                        setMenuStatus(prev => ({ ...prev, data: getMenuItemsByValue(res.data.tickets, 'state_id') }));
                        setMenuCustomer(prev => ({ ...prev, data: getMenuItemsByValue(res.data.tickets, 'customer_name') }));
            ...
            <div className="tickets-container__header">
                ...
                <div className="tickets-container__header-item problem">
                    <input 
                        ...
                        value={problemValue}
                        onChange={(e) => setProblemValue(prev => e.target.value)}
                    />
                </div>
                {/* <div className="tickets-container__header-item__separator"></div> */}
